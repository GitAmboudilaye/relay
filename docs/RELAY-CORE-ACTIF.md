...
| **R1 — garde de clôture d'état-git** ✅ v1.23.0 | `relay-uncommitted-guard.sh` : **« `git status --porcelain` vide = condition de clôture »** (lançable en CI). Ferme le trou révélé par l'examen cross-LLM DeepSeek (produit hors-git → gate au commit inerte par omission) — un LLM ne peut plus déclarer « fait » en laissant des untracked. **Portée (décision user)** = porcelain **littéral** (untracked + modifiés + stagés ; gitignorés exclus nativement → interne préservé). **Sémantique (décision user)** = **bloquant `exit 1` par défaut**, `--warn` signal-only, `--json` ; **fail-OPEN outillage** (hors repo git → exit 0) / **fail-CLOSED finding**. Pur Bash, propagé via `relay-init`, step CI miroir. | noyau (gate clôture) | — |
