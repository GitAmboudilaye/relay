...
> 2. **L'examen cross-LLM DeepSeek est fait** (examen interne ; preuve 4/10→5/10, §11). Il a fait émerger **R1** (ci-dessous).
...
Le trou révélé chez DeepSeek (produit hors-git → gate au commit inerte par omission) appelle un garde-fou neuf : **`git status --porcelain` vide = condition de clôture**, outillé par un `relay-uncommitted-guard.sh` lançable aussi en CI. Ferme exactement ce trou : un LLM ne peut plus déclarer « fait » en laissant des fichiers untracked. Arbitré à coder dans une session dédiée (pas la clôture doc).
